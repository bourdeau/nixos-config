...
# OBS
rsync -av --delete --chmod=F644,D755 --exclude-from=$repo_dir/.gitignore
  $"($env.HOME)/.config/obs-studio/basic/profiles/ScreenRecord/"
  $"($repo_dir)/obs/config/basic/profiles/ScreenRecord/"
rsync -av --delete --chmod=F644,D755 --exclude-from=$repo_dir/.gitignore
  $"($env.HOME)/.config/obs-studio/basic/scenes/"
  $"($repo_dir)/obs/config/basic/scenes/"
rsync -av --delete --chmod=F644,D755 --exclude-from=$repo_dir/.gitignore
  $"($env.HOME)/.config/obs-studio/plugin_config/obs-websocket/"
  $"($repo_dir)/obs/config/plugin_config/obs-websocket/"
